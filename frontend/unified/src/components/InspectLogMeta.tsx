...
export const DisplayLogMeta: FC = () => {
  ...
  const {loading, error, data } = useQuery(InspectLog_Query, { variables: {visitobj:Visitobj2,name:"numpy-benchmark-l5xqk"} });
  let x:any = [];
  ...
  function makeButtonArray(Arr:any){
    return <Stack direction="row" spacing={1}> {Arr.map((subArr:any) => {
      return  <Button variant="contained" onClick = {() => openInNewTab(subArr[0])}> {subArr[1]} </Button> 
    })} </Stack>
  }
